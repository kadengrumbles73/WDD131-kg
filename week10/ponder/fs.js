...
function difficultyTemplate(rating) {
  ...
  for (let i = 1; i <= 5; i++) {
    html += i <= rating ? `<span aria-hidden="true" class="icon-boot"> 🥾</span>` : `<span aria-hidden="true" class="icon-empty">▫️</span>`;
  }
  return html + `</span>`;
}
...
function renderHikes(list) {
  hikeContainer.innerHTML = list.map(hike => hikesTemplate(hike)).join('');
}
...
function search() {
  ...
  renderHikes(filtered.sort((a, b) => a.difficulty - b.difficulty));
}
